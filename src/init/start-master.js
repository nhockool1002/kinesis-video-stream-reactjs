...
const startMaster = async function (localView, remoteView, channelName) {
  ...
  const describeSignalingChannelResponse = await KinesisVideoHelper.describeSignalingChannel(
    channelName
  )
  ...
  if (ACCEPT_SEND_VIDEO || ACCEPT_SEND_AUDIO) {
    try {
      appStore.master.localStream = await navigator.mediaDevices.getUserMedia(
        constraints
      )
  ...
  appStore.master.signalingClient.on(
    'sdpOffer',
    async (offer, remoteClientId) => {
      ...
      if (!appStore.master.peerConnectionStatsInterval) {
        appStore.master.peerConnectionStatsInterval = setInterval(
      ...
      peerConnection.addEventListener('icecandidate', ({ candidate }) => {
        if (candidate) {
          console.log(
            `[MASTER] Khởi tạo ICE candidate cho client: ${remoteClientId}`
          )
          ...
          if (appStore.useTrickleICE) {
            ...
            appStore.master.signalingClient.sendIceCandidate(
              candidate,
              remoteClientId
            )
          }
          ...
          if (!appStore.useTrickleICE) {
            ...
            appStore.master.signalingClient.sendSdpAnswer(
      ...
      if (appStore.master.localStream) {
        appStore.master.localStream.getTracks().forEach((track) => {
          peerConnection.addTrack(track, appStore.master.localStream)
        })
      }
  ...
  appStore.master.signalingClient.on(
    ...
    async (candidate, remoteClientId) => {
      ...
      const peerConnection =
        appStore.master.peerConnectionByClientId[remoteClientId]
