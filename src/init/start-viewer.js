...
const startViewer = async function (localView, remoteView, channelName) {
  ...
  if (appStore.openDataChannel) {
    appStore.viewer.dataChannel = peerConnection.createDataChannel(
      'kvsDataChannel'
    )
  ...
  appStore.viewer.peerConnectionStatsInterval = setInterval(
  ...
  appStore.viewer.signalingClient.on('open', async () => {
    ...
    if (ACCEPT_SEND_VIDEO || ACCEPT_SEND_AUDIO) {
      try {
        appStore.viewer.localStream = await navigator.mediaDevices.getUserMedia(
          constraints
        )
        appStore.viewer.localStream
          .getTracks()
          .forEach((track) =>
            peerConnection.addTrack(track, appStore.viewer.localStream)
          )
    ...
    if (appStore.useTrickleICE) {
      ...
      appStore.viewer.signalingClient.sendSdpOffer(
